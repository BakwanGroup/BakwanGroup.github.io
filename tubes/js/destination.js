function GetSortOrder(prop,param) {
    if(param == 'asc') {
        return function(a, b) {    
            if (a[prop] > b[prop]) {    
                return 1;
...
function GetSortOrderLoc(prop,param) {    
    if(param == 'asc') {
        return function(a, b) {    
            if (a["loc"][prop] > b["loc"][prop]) {    
                return 1;
            } else if (a["loc"][prop] < b["loc"][prop]) {    
                return -1;
            }    
            return 0;    
        }
...
function SortOrder(prop,param) {
    var element = "";

    if(param != 'all')
        datatosort.sort(GetSortOrder(prop,param));
    $.each(datatosort, function(key, value){
            ...
            link = `http://localhost/TravelixTemplate/destination-detail.html?destination=${value.name}`;
...
function SortOrderLoc(prop,param) {
    var element = "";

    if(param != 'all')
        datatosort.sort(GetSortOrderLoc(prop,param));
    var display;
    $.each(datatosort, function(key, value){
            address = (typeof value.loc.address === 'undefined') ? "Tidak ada informasi terkait alamat" : value.loc.address;
            prov = (typeof value.loc.prov === 'undefined') ? "Tidak ada" : value.loc.prov;
